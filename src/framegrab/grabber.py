...
class FrameGrabber(ABC, BaseModel):
    ...
    def get_input_type_to_class(self):
        input_type_to_class = {
    ...
    def to_dict(self):
        ...
        input_type = next(key for key, value in self.get_input_type_to_class().items() if value == self.__class__)
    ...
    @classmethod
    def from_dict(cls, dictionary_config: dict, subclass: type ) -> "FrameGrabber":
        dictionary_config.pop("input_type")
    ...
    def create_grabber(config: dict, autogenerate_name: bool = True, warmup_delay: float = 1.0):
        ...
        grabber_class = self.get_input_type_to_class().get(input_type)
...
class WithSerialNumberMixin(BaseModel):
    ...
    def from_dict(cls, data: dict):
        ...
        if "id" in data and "serial_number" in data["id"]:
            new_data["serial_number"] = data["id"].pop("serial_number")
        return cls(**new_data)

class WithResolutionMixin(BaseModel):
    ...
    def from_dict(cls, data: dict):
        ...
        if "resolution" in options:
            new_data["resolution_width"] = options.pop("resolution").get("width")
            new_data["resolution_height"] = options.pop("resolution").get("height")
...
class GenericUSBFrameGrabber(FrameGrabber, WithSerialNumberMixin, WithResolutionMixin):
    ...
    def to_dict(self) -> dict:
        ...
        dictionary_config_serial_number = WithSerialNumberMixin.to_dict(self)
        dictionary_config_resolution = WithResolutionMixin.to_dict(self)
        dictionary_config.update(dictionary_config_serial_number)
        dictionary_config.update(dictionary_config_resolution)
        return dictionary_config
    
    @classmethod
    def from_dict(cls, data: dict):
        serial_number_data = WithSerialNumberMixin.from_dict(data)
        resolution_data = WithResolutionMixin.from_dict(data)
        framegrabber_data = FrameGrabber.from_dict(data)
        
        merged_data = {**serial_number_data.dict(), **resolution_data.dict(), **framegrabber_data.dict()}
...
class RTSPFrameGrabber(FrameGrabber):
    ...
    def from_dict(cls, data: dict):
        ...
        return FrameGrabber.from_dict(dictionary_config=new_data, subclass=cls)
...
class RealSenseFrameGrabber(FrameGrabber, WithSerialNumberMixin, WithResolutionMixin):
...
class MockFrameGrabber(FrameGrabber, WithSerialNumberMixin, WithResolutionMixin):
