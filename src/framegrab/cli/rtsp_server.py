        ...
        class RTSPMediaFactory(GstRtspServer.RTSPMediaFactory):
            ...
            def on_need_data(self, src, length):
                try:
                    ...
                    # Add frame counter
                    cv2.putText(frame, f'Frame {self.rtsp_server.frame_count}', 
...
def run_docker_rtsp_server(source: str, input_type: str | None, port: int, mount_point: str, fps: int):
    ...
    try:
            ...
            if input_type:
                config = {
                    'image_sources': [{
                        ...
                        'id': _parse_source_to_id(source, input_type)
                    }]
                }
            else:
                # Try to auto-detect
                ...
                config = {
                    'image_sources': [grabber.config.to_framegrab_config_dict()]
                }
                grabber.release()
        ...
        result = subprocess.run(['docker', 'images', '-q', 'rtsp-publisher'], 
                              capture_output=True, text=True)
        if not result.stdout.strip():
            click.echo("Building Docker image...")
            subprocess.run(['docker', 'build', '-t', 'rtsp-publisher', '.'], 
        ...
        docker_cmd = [
            ...
            'rtsp-publisher',
            'python3', 'rtsp_publisher.py', f'/app/config/{config_name}'
        ]
...
@click.option(
    "-i",
    "--input-type", 
    type=click.Choice(['generic_usb', 'rtsp', 'basler', 'realsense', 'file_stream', 'hls', 'youtube_live'], case_sensitive=False),
    default=None,
...
@click.option(
    ...
    type=int,
    default=30,
    help="Target FPS for RTSP stream (default: 30)"
)
@click.option(
    "--docker",
    is_flag=True,
    help="Force use of Docker container (auto-detected if GStreamer unavailable)"
)
